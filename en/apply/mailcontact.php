...
$site_top = "https://ufp.jp/en/";

// 管理者メールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください 例 $to = "aa@aa.aa,bb@bb.bb";)
//$to = "ufp_apply@ut-ec.co.jp,w.iwashina@frontier-ad.co.jp,n.yoshida@frontier-ad.co.jp";
$to = "ufp_apply@ut-ec.co.jp";

//自動返信メールの送信元メールアドレス
//必ず実在するメールアドレスでかつ出来る限り設置先サイトのドメインと同じドメインのメールアドレスとすることを強く推奨します
$from = "ufp_apply@ut-ec.co.jp";
...
$subject = "Program application(English)：Equity Track";
...
$re_subject = "Program application(English)：Equity Track";

//フォーム側の「名前」箇所のname属性の値　※自動返信メールの「○○様」の表示で使用します。
//指定しない、または存在しない場合は、○○様と表示されないだけです。あえて無効にしてもOK
$dsp_name = 'Name of the Applicant';

//自動返信メールの冒頭の文言 ※日本語部分のみ変更可
$remail_text = <<< TEXT

＜Program application：Equity Track＞

Thank you for your application to the UFP Equity Track.
We will respond within 3 business days in 1-2 weeks.

If you have reason to require a quicker response, please contact us directly.
The below is a receipt of your application.
TEXT;
...
South Clinical Research Bldg. 3F, 7-3-1 Hongo, Bunkyo-ku, Tokyo 113-8485, JAPAN
TEL：03-5844-6671 　FAX：03-5844-6672
E-mail: ufp_apply@ut-ec.co.jp
URL: https://ufp.jp/en/
＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
...
//配列の形「name="○○[]"」の場合には必ず後ろの[]を取ったものを指定して下さい。
$hankaku_array = array('Phone-number','Email','Confirmation of Email ID');
...
$ConfirmEmail = "Confirmation of Email ID";
...
//$permission_file = array('jpg','jpeg','gif','png','pdf');
$permission_file = array('pdf');
		...
		if (@is_uploaded_file($_FILES[$upfile_key]["tmp_name"][$i])) {
			if ($_FILES[$upfile_key]["size"][$i] < $maxImgSize) {
				...
				if($upfile_name_check != 'checkOK'){
				  $errm .= "<p class=\"error_messe\">This is an unauthorized extension.</p>\n";
				  $empty_flag = 1;
				}else{
					  ...
					  if(count($temp_file_name_array[$i]) < 2){
						$errm .= "<p class=\"error_messe\">The file has no extension.</p>\n";
						$empty_flag = 1;
					  }else{
...
if(empty($errm)){
	...
	if(!empty($post_mail) && !empty($post_mail2) && $post_mail != $post_mail2 && $mail_2check == 1){
			  $errm .= "<p class=\"error_messe\">The confirmation email addresses do not match.</p>\n";
			  $empty_flag = 1;
	}
	//----------------------------------------------------------------------
	//  メール2重チェック(BEGIN)
...
<?php }else{ ?>
<h3 class="applycheckh3">Confirmation screen</h3>
...
<input class="asend" type="submit" value="　Send　">
<input class="aback" type="button" value="Back" onclick="history.back()"></p>
</form>
...
<br /><br /><input type="button" value=" Return to the previous screen " onclick="history.back()">
</div>
</body>
</html>
<?php }else{ ?>
Thank you for your application to the UFP Equity Track.<br>
We will respond within 3 business days in 1-2 weeks.<br><br>
<a href="<?php echo $site_top ;?>">go back to the top page</a>
</div>
<?php copyright(); ?>
...
//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	...
	if($empty_flag == 1){ ?>
<div align="center"><h4>There is an error in the input. Please check the following and click the "Back" button to make corrections.</h4><div style="color:red"><?php echo $errm; ?></div><br /><br /><input type="button" value=" 前画面に戻る " onclick="history.back()"></div>
<?php 
...
function requireCheck($require){
	...
	foreach($require as $requireVal){
		...
		foreach($_POST as $key => $val) {
			if($key == $requireVal) {
				...
				if(is_array($val)){
					...
					if($connectEmpty > 0){
						$res['errm'] .= "<p class=\"error_messe\">【".h($key)."】Required fields.</p>\n";
						$res['empty_flag'] = 1;
					}
				}
				//デフォルト必須チェック
		...
		if($existsFalg != 1){
				$res['errm'] .= "<p class=\"error_messe\">【".$requireVal."】Not selected.</p>\n";
				$res['empty_flag'] = 1;
		}
	}
